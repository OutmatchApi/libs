...
export class UserApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async userConnectStripe(_options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // Path Params
        const localVarPath = '/user/connectStripe';
...
export class UserApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to userConnectStripe
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async userConnectStripe(response: ResponseContext): Promise<StripeLinkedAccount > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: StripeLinkedAccount = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "StripeLinkedAccount", ""
