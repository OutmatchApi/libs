  ...
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ObjsSubteam.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ObjsSubteam` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArrayautoType.size(); i++) {
        ObjsSubteamAutoTypeInner.validateJsonObject(jsonArrayautoType.get(i).getAsJsonObject());
      };
      ...
      for (int i = 0; i < jsonArraydeletedBy.size(); i++) {
        ObjsSubteamDeletedByInner.validateJsonObject(jsonArraydeletedBy.get(i).getAsJsonObject());
      };
