...
public class ObjsComment {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ObjsComment
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ObjsComment.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ObjsComment.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ObjsComment` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
