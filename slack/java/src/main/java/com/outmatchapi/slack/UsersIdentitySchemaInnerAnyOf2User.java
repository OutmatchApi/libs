...
public class UsersIdentitySchemaInnerAnyOf2User {
  ...
  * @throws IOException if the JSON Object is invalid with respect to UsersIdentitySchemaInnerAnyOf2User
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!UsersIdentitySchemaInnerAnyOf2User.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!UsersIdentitySchemaInnerAnyOf2User.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UsersIdentitySchemaInnerAnyOf2User` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
