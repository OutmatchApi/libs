...
public class ObjsMessage {
  ...
  private List<ObjsMessageBotIdInner> botId = null;
  ...
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ObjsMessage.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ObjsMessage` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("blocks") != null && !jsonObj.get("blocks").isJsonArray()) {
      ...
      if (jsonObj.get("bot_id") != null && !jsonObj.get("bot_id").isJsonNull()) {
        ...
        if (jsonArraybotId != null) {
          ...
          for (int i = 0; i < jsonArraybotId.size(); i++) {
            ObjsMessageBotIdInner.validateJsonObject(jsonArraybotId.get(i).getAsJsonObject());
          };
        }
      }
      ...
      // ensure the optional json data is an array if present
      if (jsonObj.get("reactions") != null && !jsonObj.get("reactions").isJsonArray()) {
