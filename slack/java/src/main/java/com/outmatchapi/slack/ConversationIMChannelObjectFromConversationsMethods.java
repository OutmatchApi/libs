...
public class ConversationIMChannelObjectFromConversationsMethods {
  ...
  private Set<ConversationIMChannelObjectFromConversationsMethodsSharesInner> shares = null;
  ...
  * @throws IOException if the JSON Object is invalid with respect to ConversationIMChannelObjectFromConversationsMethods
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ConversationIMChannelObjectFromConversationsMethods.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ConversationIMChannelObjectFromConversationsMethods.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ConversationIMChannelObjectFromConversationsMethods` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("shares") != null && !jsonObj.get("shares").isJsonNull()) {
        ...
        if (jsonArrayshares != null) {
          ...
          for (int i = 0; i < jsonArrayshares.size(); i++) {
            ConversationIMChannelObjectFromConversationsMethodsSharesInner.validateJsonObject(jsonArrayshares.get(i).getAsJsonObject());
          };
        }
      }
