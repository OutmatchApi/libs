  ...
  class BotsInfoSchemaBot
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `slack::BotsInfoSchemaBot` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `slack::BotsInfoSchemaBot`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @id !~ Regexp.new(/^B[A-Z0-9]{8,}$/)
      return false if @name.nil?
      return false if @updated.nil?
      return false if !@user_id.nil? && @user_id !~ Regexp.new(/^[UW][A-Z0-9]{2,}$/)
      true
    end
    ...
    def hash
      [app_id, deleted, icons, id, name, updated, user_id].hash
    end
