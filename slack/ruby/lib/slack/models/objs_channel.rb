  ...
  class ObjsChannel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `slack::ObjsChannel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `slack::ObjsChannel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @id !~ Regexp.new(/^[C][A-Z0-9]{2,}$/)
      ...
      return false if @members.length < 0
    ...
    def hash
      [accepted_user, created, creator, id, is_archived, is_channel, is_frozen, is_general, is_member, is_moved, is_mpim, is_non_threadable, is_org_shared, is_pending_ext_shared, is_private, is_read_only, is_shared, is_thread_only, last_read, latest, members, name, name_normalized, num_members, pending_shared, previous_names, priority, purpose, topic, unlinked, unread_count, unread_count_display].hash
    end
