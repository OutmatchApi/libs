  ...
  class ObjsUserInnerAnyOf1
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `slack::ObjsUserInnerAnyOf1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `slack::ObjsUserInnerAnyOf1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [color, deleted, enterprise_user, has_2fa, id, is_admin, is_app_user, is_bot, is_external, is_forgotten, is_owner, is_primary_owner, is_restricted, is_stranger, is_ultra_restricted, locale, name, presence, profile, real_name, team_id, team_profile, teams, two_factor_type, tz, tz_label, tz_offset, updated].hash
    end
