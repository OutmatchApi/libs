  ...
  class ObjsReminder
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `slack::ObjsReminder` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `slack::ObjsReminder`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'complete_ts')
        self.complete_ts = attributes[:'complete_ts']
      end
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^Rm[A-Z0-9]{8,}$/)
      if @id !~ pattern
    ...
    def valid?
      ...
      return false if @id !~ Regexp.new(/^Rm[A-Z0-9]{8,}$/)
    ...
    def hash
      [complete_ts, creator, id, recurring, text, time, user].hash
    end
