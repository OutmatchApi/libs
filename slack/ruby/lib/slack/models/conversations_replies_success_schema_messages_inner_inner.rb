...
module slack
  class ConversationsRepliesSuccessSchemaMessagesInnerInner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `slack::ConversationsRepliesSuccessSchemaMessagesInnerInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `slack::ConversationsRepliesSuccessSchemaMessagesInnerInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @subscribed.nil?
        invalid_properties.push('invalid value for "subscribed", subscribed cannot be nil.')
      end
      ...
      if @thread_ts.nil?
        invalid_properties.push('invalid value for "thread_ts", thread_ts cannot be nil.')
      end

      pattern = Regexp.new(/^\d{10}\.\d{6}$/)
      if @thread_ts !~ pattern
      ...
      if @parent_user_id.nil?
        invalid_properties.push('invalid value for "parent_user_id", parent_user_id cannot be nil.')
      end

      pattern = Regexp.new(/^[UW][A-Z0-9]{2,}$/)
      if @parent_user_id !~ pattern
    ...
    def valid?
      ...
      return false if @subscribed.nil?
      return false if !@team.nil? && @team !~ Regexp.new(/^[T][A-Z0-9]{2,}$/)
      return false if @text.nil?
      return false if @thread_ts.nil?
      return false if @thread_ts !~ Regexp.new(/^\d{10}\.\d{6}$/)
      ...
      return false if @parent_user_id.nil?
      return false if @parent_user_id !~ Regexp.new(/^[UW][A-Z0-9]{2,}$/)
      _any_of_found = false
    ...
    def thread_ts=(thread_ts)
      if thread_ts.nil?
        fail ArgumentError, 'thread_ts cannot be nil'
      end
    ...
    def parent_user_id=(parent_user_id)
      if parent_user_id.nil?
        fail ArgumentError, 'parent_user_id cannot be nil'
      end
    ...
    def hash
      [last_read, latest_reply, reply_count, reply_users, reply_users_count, source_team, subscribed, team, text, thread_ts, ts, type, unread_count, user, user_profile, user_team, is_starred, parent_user_id].hash
    end
