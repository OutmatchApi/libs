  ...
  class ConversationsRepliesSuccessSchemaMessagesInnerInnerAnyOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `slack::ConversationsRepliesSuccessSchemaMessagesInnerInnerAnyOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `slack::ConversationsRepliesSuccessSchemaMessagesInnerInnerAnyOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [last_read, latest_reply, reply_count, reply_users, reply_users_count, source_team, subscribed, team, text, thread_ts, ts, type, unread_count, user, user_profile, user_team].hash
    end
