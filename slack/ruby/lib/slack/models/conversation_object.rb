  ...
  class ConversationObject
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `slack::ConversationObject` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `slack::ConversationObject`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      is_mpim_validator = EnumAttributeValidator.new('Boolean', ["false"])
    ...
    def hash
      [accepted_user, connected_team_ids, conversation_host_id, created, creator, display_counts, enterprise_id, has_pins, id, internal_team_ids, is_archived, is_channel, is_ext_shared, is_frozen, is_general, is_global_shared, is_group, is_im, is_member, is_moved, is_mpim, is_non_threadable, is_open, is_org_default, is_org_mandatory, is_org_shared, is_pending_ext_shared, is_private, is_read_only, is_shared, is_starred, is_thread_only, last_read, latest, members, name, name_normalized, num_members, parent_conversation, pending_connected_team_ids, pending_shared, pin_count, previous_names, priority, purpose, shared_team_ids, shares, timezone_count, topic, unlinked, unread_count, unread_count_display, use_case, user, version].hash
    end
