  ...
  class TeamAccessLogsSchemaLoginsInner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `slack::TeamAccessLogsSchemaLoginsInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `slack::TeamAccessLogsSchemaLoginsInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'date_last')
        self.date_last = attributes[:'date_last']
      end
    ...
    def list_invalid_properties
      ...
      if @date_last.nil?
        invalid_properties.push('invalid value for "date_last", date_last cannot be nil.')
      end
    ...
    def hash
      [count, country, date_first, date_last, ip, isp, region, user_agent, user_id, username].hash
    end
