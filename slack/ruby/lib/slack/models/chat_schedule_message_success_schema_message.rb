  ...
  class ChatScheduleMessageSuccessSchemaMessage
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `slack::ChatScheduleMessageSuccessSchemaMessage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `slack::ChatScheduleMessageSuccessSchemaMessage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @bot_id !~ Regexp.new(/^B[A-Z0-9]{8,}$/)
      return false if @team.nil?
      return false if @team !~ Regexp.new(/^[T][A-Z0-9]{2,}$/)
    ...
    def bot_id=(bot_id)
      if bot_id.nil?
        fail ArgumentError, 'bot_id cannot be nil'
      end
    ...
    def hash
      [bot_id, bot_profile, team, text, type, user, username].hash
    end
