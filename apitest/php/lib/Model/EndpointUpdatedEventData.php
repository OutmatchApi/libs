...
class EndpointUpdatedEventData implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    */
     ...
     * Sets endpoint_uid
     *
     * @param string|null $endpoint_uid Optional unique identifier for the endpoint
     *
     * @return self
     */
    public function setEndpointUid($endpoint_uid)
    {
        if (!is_null($endpoint_uid) && (mb_strlen($endpoint_uid) > 256)) {
        ...
        if (!is_null($endpoint_uid) && (mb_strlen($endpoint_uid) < 1)) {
        ...
        if (!is_null($endpoint_uid) && (!preg_match("/^[a-zA-Z0-9\\-_.]+$/", $endpoint_uid))) {
        ...
        if (is_null($endpoint_uid)) {
            array_push($this->openAPINullablesSetToNull, 'endpoint_uid');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('endpoint_uid', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
