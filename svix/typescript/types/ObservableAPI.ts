...
export class ObservableAuthenticationApi {
     ...
     * @param idempotencyKey The request&#39;s idempotency key
     */
    public expireAllApiV1AuthAppAppIdExpireAllPost(appId: string, applicationTokenExpireIn: ApplicationTokenExpireIn, idempotencyKey?: string, _options?: Configuration): Observable<void> {
        const requestContextPromise = this.requestFactory.expireAllApiV1AuthAppAppIdExpireAllPost(appId, applicationTokenExpireIn, idempotencyKey, _options);
        ...
        return middlewarePreObservable.pipe(mergeMap((ctx: RequestContext) => this.configuration.httpApi.send(ctx))).
            pipe(mergeMap((response: ResponseContext) => {
                ...
                return middlewarePostObservable.pipe(map((rsp: ResponseContext) => this.responseProcessor.expireAllApiV1AuthAppAppIdExpireAllPost(rsp)));
            }));
    }
