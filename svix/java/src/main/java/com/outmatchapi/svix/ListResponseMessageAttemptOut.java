...
public class ListResponseMessageAttemptOut {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ListResponseMessageAttemptOut
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ListResponseMessageAttemptOut.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ListResponseMessageAttemptOut.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ListResponseMessageAttemptOut` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArraydata.size(); i++) {
        MessageAttemptOut.validateJsonObject(jsonArraydata.get(i).getAsJsonObject());
      };
