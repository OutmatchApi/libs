...
public class ListResponseApplicationOut {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ListResponseApplicationOut
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ListResponseApplicationOut.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ListResponseApplicationOut.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ListResponseApplicationOut` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArraydata.size(); i++) {
        ApplicationOut.validateJsonObject(jsonArraydata.get(i).getAsJsonObject());
      };
